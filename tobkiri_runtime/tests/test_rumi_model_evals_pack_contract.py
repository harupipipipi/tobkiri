...
def test_model_evals_pack_required_docs_and_assets_exist():
    required = [
        ...
        PACK_DIR / "catalog" / "provider_eval_catalog.json",
...
def test_model_evals_pack_json_and_yaml_parse():
    ...
    for path in _yaml_files():
        ...
        assert isinstance(data, dict), path
        assert any(
            key.endswith("_id") or key in {"id", "preset_id", "profile_id", "spec_id", "recipe_id"}
            for key in data
        ), path
...
def test_model_evals_pack_setup_metadata_and_selector_discoverability():
    ...
    assert setup["overlap_policy"]["defaultspack_provider_catalog"] == "overlay_with_evidence_only"
...
def test_model_evals_profiles_and_presets_are_local_first_network_none():
    for path in sorted((PACK_DIR / "profiles").glob("*.profile.yaml")):
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
        policy = data.get("policy", {})

        assert data["profile_id"].startswith("rumi_model_evals.")
