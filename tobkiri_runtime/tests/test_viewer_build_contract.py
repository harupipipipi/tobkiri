...
PANEL_SESSION_CAPABILITY = (
    TAURI_ROOT / "capabilities" / "panel-session-reauthorization.json"
)
PLATFORM_TARGETS = {
    "windows": ["nsis"],
    "macos": ["dmg"],
...
def test_remote_panel_can_only_invoke_the_declared_session_renewal_command():
    build_source = BUILD_RS.read_text(encoding="utf-8")
    capability = _read_json(PANEL_SESSION_CAPABILITY)

    assert '"reauthorize_panel_session"' in build_source
    assert capability["windows"] == ["main"]
    assert capability["permissions"] == ["allow-reauthorize-panel-session"]
...
def test_unsigned_shell_exception_is_bound_to_exact_generated_development_artifacts():
    source = (
        ROOT / "tobkiri_runtime" / "core_runtime" / "bootstrap" / "production_v4.py"
    ).read_text(encoding="utf-8")

    assert 'os.environ.get("RUMI_ENVIRONMENT") != "development"' in source
    assert 'os.environ.get("RUMI_APP_DIR")' in source
    assert '"dev-defaults"' in source
...
def test_development_runtime_install_includes_ecosystem_namespace():
    pyproject = DEV_PYPROJECT.read_text(encoding="utf-8")

    assert '"ecosystem*"' in pyproject
