...
The dispatch pipeline journals its durable markers — final authority
recheck, single-use lease consumption and audit ``dispatched`` — in the
calling thread *before* the provider Future is submitted.  Queued work is
fenced at the executor entry gate, which re-checks the shared cancellation
signal and the request deadline immediately before provider entry.
"""
...
from tobkiri_host.errors import RequestTimedOutError
...
@pytest.mark.parametrize("signal", ["parent", "drain"])
def test_queued_request_cancellation_never_enters_provider(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, signal: str,
) -> None:
    """Cancellation after enqueue fences the worker at the entry gate.

    The caller-side dispatch transaction may already have consumed the
    single-use lease when the signal lands; the entry gate still guarantees
    the provider itself is never invoked.
    """
    ...
    parent = Event() if signal == "parent" else None
    with _blocked_worker(broker) as (_release, submitted):
        with ThreadPoolExecutor(max_workers=1) as caller:
            ...
            if signal == "parent":
                ...
                broker.cancel_pending_requests()
            with pytest.raises(RequestCancellationRequestedError):
                result.result(timeout=2)
    assert not invoked.is_set()
    # The queued Future was cancelled before it ran, so no authenticated
    # backend cancel was required and the audit trail ends unexecuted.
    assert not cancelled.is_set()
    assert harness.store.audit_events()[-1]["event_state"] == "failed"


def test_deadline_elapsed_in_queue_never_enters_provider(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A fake clock proves expiry is re-checked inside the actual worker.

    Dispatch markers are journaled in the caller while the worker is still
    blocked; once released, the executor entry gate observes the expired
    deadline and terminates before provider entry.
    """
    harness = _Harness(tmp_path)
    adapter = _adapter(harness)
    broker = _broker(harness, adapter, ProviderOutcome({"ok": True}))
    ...
    with _blocked_worker(broker) as (release, submitted):
        with ThreadPoolExecutor(max_workers=1) as caller:
            result = caller.submit(
                ...
                effect_scope=harness.scope.to_dict(),
                monotonic_clock=lambda: clock[0], before_dispatch=None,
            )
            assert submitted.wait(2)
            clock[0] = 21.0
            release.set()
            with pytest.raises(RequestTimedOutError):
                result.result(timeout=2)
    assert not invoked.is_set()
    # The consumed lease settles as a recorded, non-ambiguous failure.
    assert harness.store.audit_events()[-1]["event_state"] == "failed"
...
def test_queue_timeout_is_known_unexecuted_and_cancels_future(
    ...
    with _blocked_worker(broker) as (_release, submitted):
        with ThreadPoolExecutor(max_workers=1) as caller:
            result = caller.submit(
                broker.invoke, replace(_frame(), timeout_ms=500),
...
def test_worker_guards_and_provider_keep_callers_context(
    ...
    """The executor entry gate preserves authenticated ambient context."""
    harness = _Harness(tmp_path)
    adapter = _adapter(harness)
    broker = _broker(harness, adapter, ProviderOutcome({"ok": True}))
...
def test_parent_cancellation_while_queued_never_enters_provider() -> None:
    ...
    # Recheck and dispatch markers already landed in the caller before the
    # signal; the queued Future was cancelled, so the termination is
    # recorded unexecuted rather than ambiguous.
    assert fixture.audit.failures == [("provider_failed", False)]
    assert fixture.authority.fenced == ["request-1"]


def test_parent_cancellation_during_final_recheck_retains_then_releases(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A cancel observed by the caller-side final gate terminates unexecuted.

    The lease is already consumed once the recheck returns, so the
    termination is recorded as a non-ambiguous failure while
    admission/materialization are released through the normal drain.
    """
    from tobkiri_host.errors import RequestCancellationRequestedError
    from tests.test_tobkiri_host_execution_integration import (
        context, frame, make_broker,
    ...
    original_recheck = fixture.authority.recheck_effect_boundary
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as caller:
            ...
            with pytest.raises(RequestCancellationRequestedError):
                result.result(timeout=2)
            assert fixture.backend.invocations == 0
            # No Future was ever submitted, so nothing is in flight and no
            # authenticated backend cancel is required; the termination is
            # still recorded as a non-ambiguous failure.
            assert fixture.backend.cancelled == []
            assert fixture.audit.failures == [("provider_failed", False)]
...
def test_started_external_effect_is_reconciled_after_interruption(
    monkeypatch: pytest.MonkeyPatch, interruption: str,
) -> None:
    """A started external effect stays uncertain while cancellation is outstanding."""
    ...
    fixture = make_broker(effect=EffectClass.EXTERNAL_EFFECT, timeout_ms=10000)
