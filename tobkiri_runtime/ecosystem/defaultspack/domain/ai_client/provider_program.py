...
PROVIDER_PROGRAM_RECORDS = (
    ...
    ("alibaba-dashscope", "Alibaba DashScope / Qwen", "hosted_expansion", "regional_models_api_or_snapshot"),
    ...
    ("azure-ai-foundry", "Azure AI Foundry", "enterprise_control_plane", "project_deployment_control_plane"),
    ("azure-openai", "Azure OpenAI", "enterprise_control_plane", "deployment_control_plane"),
    ("baidu-qianfan", "Baidu Qianfan / ERNIE", "hosted_expansion", "regional_models_api_or_snapshot"),
    ...
    ("cloudflare-ai-gateway", "Cloudflare AI Gateway", "gateway_expansion", "gateway_config_and_logs_api"),
    ...
    ("databricks-model-serving", "Databricks Model Serving", "enterprise_control_plane", "workspace_endpoint_control_plane"),
    ...
    ("gitlawb-opengateway", "Gitlawb OpenGateway", "gateway_or_regional", "contract_allowlist_or_gateway_api"),
    ("glm", "Zhipu GLM", "direct_hosted", "official_models_api_or_snapshot"),
    ("google", "Google Gemini", "direct_hosted", "native_api"),
    ("google-vertex-ai", "Google Vertex AI / Model Garden", "enterprise_control_plane", "project_region_control_plane"),
    ("groq", "Groq", "direct_hosted", "official_models_api"),
    ("helicone-gateway", "Helicone Gateway", "gateway_expansion", "gateway_config_api"),
    ("huggingface-inference", "Hugging Face Inference Providers / Endpoints", "hosted_expansion", "account_scoped_catalog"),
    ...
    ("openai_compatible", "Generic OpenAI-compatible connection", "local_or_custom", "configurable_api_or_manual"),
    ...
    ("oracle-oci-generative-ai", "Oracle OCI Generative AI", "enterprise_control_plane", "regional_control_plane"),
    ("perplexity", "Perplexity", "direct_hosted", "generated_official_snapshot"),
    ("portkey-ai-gateway", "Portkey AI Gateway", "gateway_expansion", "gateway_catalog_and_config_api"),
    ...
    ("text-generation-webui", "text-generation-webui", "selfhosted_expansion", "local_server_api_or_manual"),
    ("together", "Together AI", "direct_hosted", "official_models_api"),
    ("upstage", "Upstage", "direct_hosted", "official_models_api_or_snapshot"),
    ("vercel-ai-gateway", "Vercel AI Gateway", "gateway_or_regional", "public_and_account_gateway_api"),
    ...
    ("xiaomi-token-plan-ams", "Xiaomi Token Plan AMS", "gateway_or_regional", "plan_scoped_inventory"),
    ("xiaomi-token-plan-cn", "Xiaomi Token Plan CN", "gateway_or_regional", "plan_scoped_inventory"),
    ("xiaomi-token-plan-sgp", "Xiaomi Token Plan SGP", "gateway_or_regional", "plan_scoped_inventory"),
)
...
def provider_program_manifests() -> Dict[str, Dict[str, Any]]:
    ...
    for provider_id, display_name, family, inventory_strategy in PROVIDER_PROGRAM_RECORDS:
        kind = "local" if family in {"local_or_custom", "selfhosted_expansion"} else (
            "enterprise" if family == "enterprise_control_plane" else (
                "gateway" if "gateway" in family else "cloud"
            )
        )
