...
class AnthropicProvider(BaseProvider):
    ...
    def _live_model_record(raw):
        ...
        thinking = capabilities.get("thinking") if isinstance(capabilities.get("thinking"), dict) else {}
        effort = capabilities.get("effort") if isinstance(capabilities.get("effort"), dict) else {}
        levels = [
            level for level in ("low", "medium", "high", "xhigh")
        ...
        for _ in range(100):
            ...
            for raw in entries if isinstance(entries, list) else []:
                ...
                if model and all(item["model_id"] != model["model_id"] for item in models):
                    models.append(model)
            next_cursor = str(page.get("last_id") or "").strip() if isinstance(page, dict) and page.get("has_more") else ""
