...
class AzureOpenAIProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        self._api_version = str(os.environ.get("AZURE_OPENAI_API_VERSION", self.DEFAULT_API_VERSION) or self.DEFAULT_API_VERSION).strip()
        self._ssl_ctx = ssl.create_default_context()

    @classmethod
    def _configured_base_url(cls) -> str:
        configured = str(os.environ.get("AZURE_OPENAI_ENDPOINT") or os.environ.get("AZURE_OPENAI_BASE_URL") or "").strip()
        if configured:
    ...
    def _headers(self) -> Dict[str, str]:
        return {"api-key": self._api_key, "Accept": "application/json", "Content-Type": "application/json", "User-Agent": "RumiAI/1.0"}
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if not self._base_url:
            raise RuntimeError("azure-openai: configure AZURE_OPENAI_ENDPOINT or an endpoint URL on the saved API")
        data = json.dumps(body).encode("utf-8") if body is not None else None
        request = urllib.request.Request(self._base_url + path, data=data, headers=self._headers(), method=method)
        try:
            with urllib.request.urlopen(request, context=self._ssl_ctx, timeout=120) as response:
                payload = response.read().decode("utf-8")
        except urllib.error.HTTPError as error:
            raise RuntimeError(f"Azure OpenAI API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def _deployment_record(cls, raw: Any) -> Dict[str, Any] | None:
        ...
        if not deployment:
            return None
        model = raw.get("model") if isinstance(raw.get("model"), dict) else raw.get("properties", {}).get("model", {}) if isinstance(raw.get("properties"), dict) else {}
        ...
        return {
            ...
            "name": deployment,
            "display_name": f"{deployment} ({model_name})" if model_name != deployment else deployment,
            "type": model_type,
            "capabilities": {
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        models = [item for raw in entries if (item := self._deployment_record(raw)) is not None] if isinstance(entries, list) else []
        if models:
    ...
    def complete(self, model, messages, tools, params):
        ...
        for key in ("stream", "max_tokens", "temperature", "top_p", "stop", "response_format"):
            if key in (params or {}):
                body[key] = params[key]
        payload = self._request_json("POST", self._api_path("/openai/deployments/" + urllib.parse.quote(deployment, safe="") + "/chat/completions"), body)
    ...
    def embed(self, model, input_text):
        ...
        payload = self._request_json("POST", self._api_path("/openai/deployments/" + urllib.parse.quote(deployment, safe="") + "/embeddings"), {"input": inputs})
        ...
        return {
            ...
            "usage": {"input_tokens": usage.get("prompt_tokens", 0), "total_tokens": usage.get("total_tokens", 0)},
        }
