...
class CohereProvider(BaseProvider):
    ...
    def __init__(self):
        self._api_key = str(os.environ.get("COHERE_API_KEY", "") or "").strip()
        self._base_url = str(os.environ.get("COHERE_BASE_URL", self.BASE_URL) or self.BASE_URL).strip().rstrip("/")
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Cohere API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def _model_record(raw: Any) -> Dict[str, Any] | None:
        ...
        if not model_id:
            return None
        endpoints = {str(item).strip().lower() for item in raw.get("endpoints", []) if str(item).strip()}
        features = {str(item).strip().lower() for item in raw.get("features", []) if str(item).strip()}
        is_embedding = bool({"embed", "embeddings"} & endpoints) or any("embed" in item for item in features)
        is_rerank = "rerank" in endpoints or any("rerank" in item for item in features)
        is_chat = bool({"chat", "chat-completions"} & endpoints) or any("chat" in item for item in features)
    ...
    def _chat_messages(self, messages: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for message in messages or []:
            ...
            if role not in {"user", "assistant", "system", "tool"}:
                role = "user"
            result.append({"role": role, "content": self._message_content(message.get("content", ""))})
        return result

    @staticmethod
    def _usage(raw: Dict[str, Any]) -> Dict[str, int]:
        usage = raw.get("usage") if isinstance(raw.get("usage"), dict) else {}
        tokens = usage.get("tokens") if isinstance(usage.get("tokens"), dict) else usage.get("billed_units", {})
    ...
    def stream(self, model, messages, tools, params):
        ...
        request = urllib.request.Request(self._base_url + "/v2/chat", data=data, headers=self._headers(), method="POST")
        try:
        ...
        try:
            for line in response:
                ...
                if event_name == "content-delta":
                    text = (((payload.get("delta") or {}).get("message") or {}).get("content") or {}).get("text")
                    if text:
