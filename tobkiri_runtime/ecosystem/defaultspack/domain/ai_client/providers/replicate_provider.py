...
class ReplicateProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        self._base_url = str(os.environ.get("REPLICATE_BASE_URL", self.BASE_URL) or self.BASE_URL).strip().rstrip("/")
    ...
    def _request_json(self, method: str, url_or_path: str, body: Dict[str, Any] | None = None, *, wait: bool = False) -> Dict[str, Any]:
        if not self._api_key:
            raise RuntimeError("replicate: missing REPLICATE_API_TOKEN")
        url = url_or_path if str(url_or_path).startswith(("https://", "http://")) else self._base_url + "/" + str(url_or_path).lstrip("/")
        ...
        try:
            ...
            raise RuntimeError(f"Replicate API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def _task_type(raw: Dict[str, Any]) -> str:
        schema = ((raw.get("latest_version") or {}).get("openapi_schema") or {}) if isinstance(raw.get("latest_version"), dict) else {}
        serialized = json.dumps(schema, ensure_ascii=False).lower() if isinstance(schema, dict) else ""
    ...
    def _model_record(cls, raw: Any) -> Dict[str, Any] | None:
        ...
        return {
            ...
            "metadata": {
                ...
                "schema": latest.get("openapi_schema") if isinstance(latest.get("openapi_schema"), dict) else {},
            },
        }
    ...
    def _input_for(self, model: str, messages: Iterable[Dict[str, Any]], params: Dict[str, Any]) -> Dict[str, Any]:
        extra = params.get("extra_body") if isinstance(params.get("extra_body"), dict) else {}
        explicit = extra.get("replicate_input") if isinstance(extra.get("replicate_input"), dict) else {}
        if explicit:
        ...
        if raw is None:
            self.list_models()
            raw = self._model_records.get(model, {})
        example = raw.get("default_example") if isinstance(raw, dict) and isinstance(raw.get("default_example"), dict) else {}
        input_data = dict(example.get("input") or {}) if isinstance(example.get("input"), dict) else {}
    ...
    def _run_prediction(self, model_id: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if status in {"starting", "processing"}:
            poll_url = ((prediction.get("urls") or {}).get("get")) if isinstance(prediction.get("urls"), dict) else ""
            for _ in range(60):
                time.sleep(1)
    ...
    def stream(self, model, messages, tools, params):
        ...
        for part in response.get("content", []):
            if isinstance(part, dict) and part.get("type") == "text" and part.get("text"):
                yield {"type": "content_delta", "delta": {"type": "text", "text": str(part["text"])}}
