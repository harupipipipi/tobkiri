...
class ElevenLabsProvider(BaseProvider):
    ...
    def __init__(self):
        self._api_key = str(os.environ.get("ELEVENLABS_API_KEY", "") or "").strip()
        self._base_url = str(os.environ.get("ELEVENLABS_BASE_URL", self.BASE_URL) or self.BASE_URL).strip().rstrip("/")
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            raise RuntimeError(f"ElevenLabs API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        models = [model for model in (self._model_record(raw) for raw in entries or []) if model is not None]
        if models:
    ...
    def tts(self, model, text, voice):
        ...
        audio = self._request_audio("/v1/text-to-speech/" + urllib.parse.quote(voice_id, safe=""), payload)
