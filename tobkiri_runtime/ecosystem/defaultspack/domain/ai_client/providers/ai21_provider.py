...
class AI21Provider(OpenAICompatibleProvider):
    ...
    def __init__(self, api_key: str = "", base_url: str = "", *, api_key_env: Any = "AI21_API_KEY", base_url_env: Any = "AI21_BASE_URL", default_base_url: str = DEFAULT_BASE_URL):
        super().__init__(api_key=api_key, base_url=base_url, provider_id=self.provider_name, display_name=self.display_name, api_key_env=api_key_env, base_url_env=str(base_url_env[0] if isinstance(base_url_env, (list, tuple)) and base_url_env else base_url_env), default_base_url=default_base_url, credential_required=True, known_models=[], remote_model_discovery=True, remote_model_discovery_requires_auth=False, remote_model_list_path="/models", remote_model_cache_ttl_seconds=86400)
    ...
    def _fetch_remote_models(self) -> List[Dict[str, Any]]:
        ...
        """
        request = urllib.request.Request(self.MODEL_DOCUMENT_URL, headers={"Accept": "text/markdown", "User-Agent": "RumiAI/1.0"}, method="GET")
        try:
        ...
        return self._normalize_remote_models([{"id": model_id, "name": model_id, "context_length": 256000, "type": "chat", "capabilities": ["chat", "stream"]} for model_id in identifiers])
    ...
    def _normalize_remote_model(self, raw: Any) -> Dict[str, Any] | None:
        ...
        if model:
            ...
            metadata.update({"source": "ai21_official_model_document", "source_endpoint": self.MODEL_DOCUMENT_URL, "capability_confidence": "official_document"})
