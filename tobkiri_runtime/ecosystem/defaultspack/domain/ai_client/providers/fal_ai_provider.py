...
class FalAIProvider(BaseProvider):
    ...
    def __init__(self) -> None:
        ...
        self._models_base_url = str(
            self._connection.get("base_url")
            or os.environ.get("FAL_API_BASE_URL")
            or self.MODELS_BASE_URL
        ).strip().rstrip("/")
        self._queue_base_url = str(
            os.environ.get("FAL_QUEUE_BASE_URL") or self.QUEUE_BASE_URL
        ).strip().rstrip("/")
    ...
    def _scope(self) -> str:
        return hashlib.sha256(
            (self._api_key + "\0" + self._models_base_url + "\0" + self._queue_base_url).encode("utf-8")
        ).hexdigest()
    ...
    def _request_json(self, method: str, url: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"fal.ai API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def _model_type(raw: Dict[str, Any]) -> str:
        ...
        category = str(metadata.get("category") or raw.get("category") or "").lower().replace("_", "-")
        if "image" in category:
            return "image_gen"
        if "video" in category:
            return "video_gen"
        if any(token in category for token in ("speech", "text-to-speech", "audio-generation", "music")):
            return "tts"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        for _ in range(1000):
            ...
            payload = self._request_json("GET", self._models_base_url + "/models" + ("?" + query if query else ""))
            ...
            cursor = next_cursor
        if records:
            self._INVENTORY_CACHE[scope] = (now + self._CACHE_TTL_SECONDS, [dict(item) for item in records])
        return records
    ...
    def _input_for(self, messages: Iterable[Dict[str, Any]], params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _run(self, model_id: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not response_url:
            ...
            return submitted
        for _ in range(120):
            status = self._request_json("GET", status_url) if status_url else {"status": "COMPLETED"}
    ...
    def image_gen(self, model, prompt, params):
        model_id = str(model or "").removeprefix("fal-ai/")
        result = self._run(model_id, self._input_for([{"role": "user", "content": str(prompt or "")}], dict(params or {})))
        return {"images": self._urls(result.get("images") or result.get("image") or result), "raw_extra": {"model": model_id}}
