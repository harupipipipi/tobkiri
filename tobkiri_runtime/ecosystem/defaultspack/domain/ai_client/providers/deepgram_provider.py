...
class DeepgramProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        self._base_url = str(os.environ.get("DEEPGRAM_BASE_URL", self.BASE_URL) or self.BASE_URL).strip().rstrip("/")
        self._ssl_ctx = ssl.create_default_context()

    def _headers(self, content_type: str = "application/json", accept: str = "application/json") -> Dict[str, str]:
        headers = {"Authorization": "Token " + self._api_key, "Accept": accept, "User-Agent": "RumiAI/1.0"}
        if content_type:
    ...
    def _request(self, method: str, path: str, body: Any = None, *, content_type: str = "application/json", accept: str = "application/json") -> bytes:
        if not self._api_key:
            raise RuntimeError("deepgram: missing DEEPGRAM_API_KEY")
        data = json.dumps(body).encode("utf-8") if content_type == "application/json" and body is not None else body
        request = urllib.request.Request(self._base_url + path, data=data, headers=self._headers(content_type, accept), method=method)
        try:
            with urllib.request.urlopen(request, context=self._ssl_ctx, timeout=120) as response:
                return response.read()
        except urllib.error.HTTPError as error:
            raise RuntimeError(f"Deepgram API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def _model_record(raw: Any, task: str) -> Dict[str, Any] | None:
        ...
        return {
            ...
            "metadata": {
                ...
                "languages": list(raw.get("languages") or []) if isinstance(raw.get("languages"), list) else [],
    ...
    def transcribe(self, model, audio, params):
        ...
        if audio_value.startswith("http://") or audio_value.startswith("https://"):
            ...
            encoded = audio_value.split(",", 1)[1] if audio_value.startswith("data:") and "," in audio_value else audio_value
            try:
                audio_bytes = base64.b64decode(encoded)
            except ValueError as error:
                raise RuntimeError("deepgram: audio must be a URL or base64 data") from error
            payload = self._request("POST", path, audio_bytes, content_type=str((params or {}).get("mime_type") or "audio/wav"))
            try:
                raw = json.loads(payload.decode("utf-8"))
            except (json.JSONDecodeError, ValueError) as error:
                raise RuntimeError(f"Deepgram API returned invalid JSON: {payload[:500]!r}") from error
        channels = (((raw.get("results") or {}).get("channels") or []) if isinstance(raw, dict) else [])
        alternatives = channels[0].get("alternatives") if channels and isinstance(channels[0], dict) else []
        transcript = str(alternatives[0].get("transcript") or "") if alternatives and isinstance(alternatives[0], dict) else ""
