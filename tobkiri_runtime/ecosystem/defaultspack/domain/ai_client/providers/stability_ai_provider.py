...
class StabilityAIProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        self._base_url = str(
            self._connection.get("base_url") or os.environ.get("STABILITY_API_BASE_URL") or self.DEFAULT_BASE_URL
        ).strip().rstrip("/")
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Dict[str, Any] | List[Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Stability AI API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        engines = payload if isinstance(payload, list) else payload.get("engines", []) if isinstance(payload, dict) else []
    ...
    def image_gen(self, model, prompt, params):
        ...
        for key in ("cfg_scale", "height", "width", "sampler", "samples", "seed", "steps", "style_preset"):
            if key in (params or {}):
                body[key] = params[key]
        payload = self._request_json("POST", "/v1/generation/" + urllib.parse.quote(engine_id, safe="-_.~") + "/text-to-image", body)
        artifacts = payload.get("artifacts") if isinstance(payload, dict) and isinstance(payload.get("artifacts"), list) else []
        images = []
