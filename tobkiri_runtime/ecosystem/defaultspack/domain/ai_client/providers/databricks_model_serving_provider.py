...
class DatabricksModelServingProvider(BaseProvider):
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if not self._base_url:
            raise RuntimeError("databricks-model-serving: configure DATABRICKS_HOST or a workspace URL on the saved API")
        ...
        try:
            ...
            raise RuntimeError(f"Databricks Model Serving API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
        except urllib.error.URLError as error:
            raise RuntimeError(f"Databricks Model Serving connection error: {error.reason}") from error
        try:
            decoded = json.loads(payload)
        except (json.JSONDecodeError, ValueError) as error:
            raise RuntimeError(f"Databricks Model Serving returned invalid JSON: {payload[:500]}") from error
    ...
    def _endpoint_record(raw: Any) -> Dict[str, Any] | None:
        ...
        served_entities = config.get("served_entities") if isinstance(config.get("served_entities"), list) else []
        served_models = config.get("served_models") if isinstance(config.get("served_models"), list) else []
        entity_names = [
            str(item.get("name") or item.get("entity_name") or "").strip()
            for item in served_entities + served_models
            if isinstance(item, dict) and str(item.get("name") or item.get("entity_name") or "").strip()
        ]
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        models = [item for endpoint in raw_endpoints if (item := self._endpoint_record(endpoint)) is not None]
        if models:
    ...
    def complete(self, model, messages, tools, params):
        ...
        for key in ("stream", "max_tokens", "temperature", "top_p", "stop", "response_format"):
            if key in (params or {}):
                body[key] = params[key]
        payload = self._request_json("POST", "/serving-endpoints/" + urllib.parse.quote(model_id, safe="") + "/invocations", body)
