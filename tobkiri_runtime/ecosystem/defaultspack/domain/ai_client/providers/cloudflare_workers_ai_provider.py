...
class CloudflareWorkersAIProvider(BaseProvider):
    ...
    def __init__(self):
        self._api_key = str(os.environ.get("CLOUDFLARE_API_TOKEN", "") or "").strip()
        self._base_url = str(os.environ.get("CLOUDFLARE_WORKERS_AI_BASE_URL", self.BASE_URL) or self.BASE_URL).strip().rstrip("/")
    ...
    def _request_json(self, method: str, path: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if not account_base:
            raise RuntimeError("cloudflare-workers-ai: configure CLOUDFLARE_ACCOUNT_ID or an account-scoped base URL")
        data = json.dumps(body).encode("utf-8") if body is not None else None
        request = urllib.request.Request(account_base + path, data=data, headers=self._headers(), method=method)
        try:
            with urllib.request.urlopen(request, context=self._ssl_ctx, timeout=120) as response:
                payload = response.read().decode("utf-8")
        except urllib.error.HTTPError as error:
            raise RuntimeError(f"Cloudflare Workers AI API error {error.code}: {error.read().decode('utf-8', errors='replace')}") from error
        except urllib.error.URLError as error:
            raise RuntimeError(f"Cloudflare Workers AI connection error: {error.reason}") from error
        try:
            decoded = json.loads(payload)
        except (json.JSONDecodeError, ValueError) as error:
            raise RuntimeError(f"Cloudflare Workers AI returned invalid JSON: {payload[:500]}") from error
        if not isinstance(decoded, dict):
            return {"result": decoded}
        if decoded.get("success") is False:
            raise RuntimeError(f"Cloudflare Workers AI API error: {decoded.get('errors') or decoded.get('messages') or 'request failed'}")
        return decoded
    ...
    def _task_name(raw: Dict[str, Any]) -> str:
        ...
        return str(task or raw.get("task_name") or raw.get("type") or "").strip().lower().replace("_", "-")
    ...
    def _model_record(cls, raw: Any) -> Dict[str, Any] | None:
        ...
        return {
            ...
            "capabilities": {
                "chat": is_chat,
                "text_input": is_chat or model_type in {"embedding", "image_gen", "tts", "classification"},
    ...
    def complete(self, model, messages, tools, params):
        ...
        if choices:
            ...
            text = str(result.get("response") or result.get("result") or "") if isinstance(result, dict) else str(result or "")
        return {"content": [{"type": "text", "text": text}], "finish_reason": "stop", "raw_extra": {"model": model_id}}
