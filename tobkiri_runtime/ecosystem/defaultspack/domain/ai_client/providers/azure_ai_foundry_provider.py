...
class AzureAIFoundryProvider(BaseProvider):
    ...
    def _request_json(self, method: str, url: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        if not self._api_key:
            raise RuntimeError("azure-ai-foundry: save an API key for the selected project connection")
        if not self._base_url:
            raise RuntimeError("azure-ai-foundry: save the Foundry Project endpoint URL on the API connection")
        ...
        try:
            ...
            raise RuntimeError(f"Azure AI Foundry returned invalid JSON: {payload[:500]}") from error
        return decoded if isinstance(decoded, dict) else {"data": decoded}

    def _project_url(self, path: str) -> str:
        separator = "&" if "?" in path else "?"
        return self._base_url + path + separator + urllib.parse.urlencode({"api-version": self._api_version})

    @staticmethod
    def _deployment_record(raw: Any) -> Dict[str, Any] | None:
        if not isinstance(raw, dict):
            return None
        deployment = str(raw.get("name") or raw.get("id") or raw.get("deploymentName") or "").strip()
        if not deployment:
            return None
        properties = raw.get("properties") if isinstance(raw.get("properties"), dict) else {}
        model = raw.get("model") if isinstance(raw.get("model"), dict) else properties.get("model", {})
        ...
        lowered = f"{deployment} {model_name} {json.dumps(capabilities)} {json.dumps(modalities)}".lower()
        model_type = "embedding" if "embed" in lowered else "image_gen" if "image" in lowered and "generation" in lowered else "chat"
        return {
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        while url and url not in visited:
            ...
            if next_url and urllib.parse.urlsplit(next_url).netloc == urllib.parse.urlsplit(self._base_url).netloc:
                url = next_url
            else:
                url = ""
        models = [item for raw in raw_deployments if (item := self._deployment_record(raw)) is not None]
        if models:
    ...
    def complete(self, model, messages, tools, params):
        ...
        for key in ("stream", "max_tokens", "temperature", "top_p", "stop", "response_format"):
            if key in (params or {}):
                body[key] = params[key]
        payload = self._request_json("POST", self._inference_url(deployment, "chat/completions"), body)
    ...
    def embed(self, model, input_text):
        ...
        payload = self._request_json("POST", self._inference_url(deployment, "embeddings"), {"input": inputs})
