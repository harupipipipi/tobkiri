...
class AwsBedrockProvider(BaseProvider):
    ...
    def _configured_credentials(cls, connection: Dict[str, Any]) -> Dict[str, str]:
        ...
        if isinstance(decoded, dict) and decoded:
            access_key_id = str(decoded.get("access_key_id") or decoded.get("accessKeyId") or "").strip()
            secret_access_key = str(decoded.get("secret_access_key") or decoded.get("secretAccessKey") or "").strip()
            session_token = str(decoded.get("session_token") or decoded.get("sessionToken") or "").strip()
        else:
    ...
    def _signed_headers(self, method: str, host: str, path: str, *, body: bytes = b"", service: str) -> Dict[str, str]:
        if not self._credentials:
            raise RuntimeError("aws-bedrock: configure AWS credentials or save an access_key_id:secret_access_key connection")
        ...
        date_key = self._sign(("AWS4" + self._credentials["secret_access_key"]).encode("utf-8"), date_stamp)
    ...
    def _request_json(self, method: str, host: str, path: str, *, service: str, body: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        request = urllib.request.Request(f"https://{host}{path}", data=payload if body is not None else None, headers=headers, method=method)
        try:
    ...
    def _model_record(raw: Any) -> Dict[str, Any] | None:
        ...
        if not model_id:
            return None
        inputs = {str(value).upper() for value in raw.get("inputModalities", []) if isinstance(value, str)}
        outputs = {str(value).upper() for value in raw.get("outputModalities", []) if isinstance(value, str)}
        model_type = "embedding" if "EMBEDDING" in outputs else "image_gen" if "IMAGE" in outputs else "chat"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        raw_models = payload.get("modelSummaries") if isinstance(payload.get("modelSummaries"), list) else []
        models = [item for raw in raw_models if (item := self._model_record(raw)) is not None]
        if models:
            self._MODEL_INVENTORY_CACHE[scope] = (now + self._MODEL_INVENTORY_CACHE_TTL_SECONDS, [dict(item) for item in models])
        return models
    ...
    def _content_part(value: Any) -> Dict[str, Any]:
        ...
        if isinstance(value, list):
            text = "".join(str(part.get("text") or "") if isinstance(part, dict) else str(part) for part in value)
            return {"text": text}
    ...
    def complete(self, model, messages, tools, params):
        ...
        for message in list(messages or []):
            ...
            if role == "system":
                ...
                converted.append({"role": "assistant" if role == "assistant" else "user", "content": [self._content_part(content)]})
        ...
        return {
            ...
            "usage": {"input_tokens": usage.get("inputTokens", 0), "output_tokens": usage.get("outputTokens", 0), "total_tokens": usage.get("totalTokens", 0)},
