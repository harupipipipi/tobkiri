...
test("account connections use vendored data images for supported brands", () => {
  for (const providerId of ["cloudflare", "google", "github"]) {
...
test("Codex uses the bundled official application icon", () => {
  const asset = providerBrandAsset("codex") ?? "";
  assert.match(asset, /codex\.png$/);
