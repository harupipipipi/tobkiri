...
export type ProviderBrandId = keyof typeof BRAND_SVGS | "codex";

export function providerBrandAsset(providerId: string): string | null {
  if (providerId === "codex") return new URL("./codex.png", import.meta.url).href;
  const svg = BRAND_SVGS[providerId as keyof typeof BRAND_SVGS];
