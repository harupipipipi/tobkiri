"""Single-write profile-bound memory owner."""
...
VERSION = "rumi.memory-store.v1"
_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$")


class MemoryConflict(RuntimeError):
    """Raised for a stale memory-store or record revision."""


class MemoryStore:
    """Own durable memory records in one atomic JSON state file."""
    ...
    def snapshot(self) -> dict[str, Any]:
        """Return the complete owner snapshot."""
        state = self._read()
        return {
            "version": VERSION,
    ...
    def get(self, memory_id: str) -> dict[str, Any] | None:
        """Return one nonexpired memory record."""
        item = self._read()["items"].get(_identifier(memory_id))
        if not isinstance(item, Mapping) or _expired(item):
            return None
        return _copy(item)

    def search(self, query: str, *, limit: int = 8) -> dict[str, Any]:
        """Search the owner state with deterministic local text scoring."""
        state = self._read()
        ranked = []
        for item in state["items"].values():
            if _expired(item):
                continue
            score = _score(query, str(item.get("content") or ""))
            if score > 0:
                ranked.append({**_copy(item), "score": score})
        ranked.sort(key=lambda item: (-item["score"], item["id"]))
        return {"revision": state["revision"], "items": ranked[: max(0, limit)]}
    ...
    def put(
        ...
        *,
        ...
        """Create or replace one record at an exact store revision."""
        normalized = _record(record)
        with NamedLock(self.lock_root, "memory"):
            state = self._read()
            _assert_revision(state, expected_revision)
            current = state["items"].get(normalized["id"])
            record_revision = int(current.get("record_revision") or 0) + 1 if current else 1
            normalized["record_revision"] = record_revision
    ...
    def delete(self, memory_id: str, *, expected_revision: int) -> dict[str, Any]:
        """Delete one record at an exact store revision."""
        memory_id = _identifier(memory_id)
        with NamedLock(self.lock_root, "memory"):
            state = self._read()
            _assert_revision(state, expected_revision)
            if memory_id not in state["items"]:
                raise KeyError("memory is unknown")
            del state["items"][memory_id]
            state["revision"] += 1
            self._write(state)
        return {"deleted_id": memory_id, "revision": state["revision"]}
    ...
    def migrate(
        ...
        *,
        ...
        if actual_hash != expected_source_hash:
            raise MemoryConflict("memory migration source hash does not match")
        with NamedLock(self.lock_root, "memory"):
            state = self._read()
            if state["items"] or state.get("migration"):
                raise MemoryConflict("memory owner is not migration-empty")
    ...
    def rollback(self, migration_id: str) -> dict[str, Any]:
        """Restore the exact owner backup bound to a migration marker."""
        ...
        with NamedLock(self.lock_root, "memory"):
            ...
            if not isinstance(marker, Mapping) or marker.get("id") != migration_id:
                raise MemoryConflict("memory migration marker does not match")
    ...
    def _normalize(self, value: Any) -> dict[str, Any]:
        if not isinstance(value, Mapping) or value.get("version") != VERSION:
            raise ValueError("memory owner state is invalid")
        if value.get("profile_id") != self.profile_id:
            raise ValueError("memory owner profile does not match")
        items = value.get("items")
        if not isinstance(items, Mapping):
            raise ValueError("memory owner records are invalid")
        return {
            "version": VERSION,
            "profile_id": self.profile_id,
...
def create_memory_resource(client: Any) -> Callable[[str, Mapping[str, Any]], Any]:
    """Create memory read operations."""
    del client

    def operation(name: str, payload: Mapping[str, Any]) -> Any:
        store = MemoryStore(_profile(payload))
        if name == "snapshot":
            return store.snapshot()
        if name == "get":
            return store.get(str(payload.get("memory_id") or ""))
        if name == "search":
            return store.search(
                str(payload.get("query") or ""),
                limit=max(0, min(100, int(payload.get("limit") or 8))),
            )
        raise ValueError(f"unknown memory resource operation: {name}")

    return operation


def create_memory_action(client: Any) -> Callable[[str, Mapping[str, Any]], Any]:
    """Create memory mutation operations."""
    del client
    ...
    def operation(name: str, payload: Mapping[str, Any]) -> Any:
        ...
        if name == "delete":
            return store.delete(
                str(payload.get("memory_id") or ""), expected_revision=expected
            )
        raise ValueError(f"unknown memory action: {name}")

    return operation


def create_migration_action(client: Any) -> Callable[[str, Mapping[str, Any]], Any]:
    """Create explicit memory migration operations."""
    del client
    ...
    def operation(name: str, payload: Mapping[str, Any]) -> Any:
        ...
        if name == "migrate":
            ...
            if not isinstance(items, list):
                raise ValueError("memory migration items are invalid")
        ...
        raise ValueError(f"unknown memory migration action: {name}")
...
def _score(query: str, content: str) -> float:
    ...
    if not query_words:
        return 1.0
    if query.casefold() in content_folded:
        return 1.0
    matches = sum(word in content_folded for word in query_words)
    return round(matches / len(query_words), 6)


def _expired(item: Mapping[str, Any]) -> bool:
    expires_at = item.get("expires_at")
    return expires_at is not None and int(expires_at) <= _now_ms()


def _assert_revision(state: Mapping[str, Any], expected: int) -> None:
    if int(state.get("revision") or 0) != expected:
        raise MemoryConflict("memory store revision is stale")
